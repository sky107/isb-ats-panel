...
const Menu = ({ currentMenuTab, sidebarToggle, mobileView }) => {
  ...
  if (currentMenuTab) {
    ...
    const isAdmin = localStorage.getItem("rfc-isAdmin");
    if (isAdmin) {
      // alert(JSON.stringify(menuItem))
       menuItem.subMenu = menuItem.subMenu.filter((item) => (item?.text === "Manage Applications" || item?.text === "Manage Jobs" ));
      // setAdminMenu(al);
      // alert(JSON.stringify(al))
    }else{
      menuItem.subMenu = menuItem.subMenu.filter((item) => item?.text !== "Manage Applications");

    }
    // menuItem=menuItem.subMenu.find(item=>item.text ==='Manage Users')
  }
